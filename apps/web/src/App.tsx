...
import './pcif.css';
import './treasury.css';
import { LogOut, BadgeEuro, BookOpenCheck, Building2, CalendarDays, CheckCircle2, Home, Landmark, LineChart, ReceiptText, RefreshCw, ShieldCheck, Upload, UsersRound, WalletCards, Settings } from 'lucide-react';
...
export default function App(){
 ...
 useEffect(()=>{api.me().then(r=>{setUser(r.user);if(r.user?.globalRole==='ADMIN')setView('Administration');void load()}).catch(()=>setUser(null))},[load]);
 if(user===undefined)return <div className="loading">Vérification de la session…</div>;
 if(!user)return <LoginView onLogin={u=>{setUser(u);if(u?.globalRole==='ADMIN')setView('Administration');void load()}}/>;
 ...
 async function upload(e:React.FormEvent<HTMLFormElement>){e.preventDefault();setResult({loading:true});try{const form=new FormData(e.currentTarget);const ets=String(form.get('ets')||'');const exercise=Number(form.get('exercise')||0)||undefined;form.delete('ets');form.delete('exercise');const x=await api.importOpale(form,ets,exercise);setResult(x);await load()}catch(error){setResult({error:error instanceof Error?error.message:'Erreur inconnue'})}}
 return <div className="shell"><aside className="sidebar"><button className="logo logo-home" type="button" title="Revenir à l’accueil · Toute l’agence" aria-label="VIGIE EPLE — revenir à l’accueil de toute l’agence" onClick={()=>{setSelectedId('all');setView(user.globalRole==='ADMIN'?'Administration':'Accueil')}}><img className="vigie-eye-logo" src="/vigie-eye.svg" alt="" aria-hidden="true"/><div><strong>VIGIE EPLE</strong><small>Observer · Analyser · Anticiper</small></div></button><nav>{user.globalRole==='ADMIN'?<button className={view==='Administration'?'active':''} onClick={()=>setView('Administration')}><Settings size={18}/>Administration</button>:NAV.map(([n,I])=><button className={view===n?'active':''} onClick={()=>setView(n)} key={n}><I size={18}/>{n}</button>)}</nav>{user.globalRole!=='ADMIN'&&<div className="side-quote">« Anticiper aujourd'hui<br/>pour sécuriser demain »</div>}<div className="side-user"><b>{[user.firstName,user.lastName].filter(Boolean).join(' ')||user.email}</b><small>{user.globalRole==='ADMIN'?'Administrateur':(user.agencies?.[0]?.role||'Utilisateur')}</small><button onClick={async()=>{await api.logout();setDash(null);setUser(null)}}><LogOut size={14}/> Déconnexion</button></div><small className="version">VIGIE v0.0.58</small></aside>
 <main className="content"><header className="topbar"><div className="est-select"><span>Établissement</span><select value={selectedId} onChange={e=>setSelectedId(e.target.value)}><option value="all">Vue agence · tous les EPLE</option>{all.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select></div><div className="situation"><span>Dernière mise à jour</span><b><CalendarDays size={16}/> {dateFr(current?.freshness||dash?.generatedAt)}</b><small><CheckCircle2 size={14}/> Données historisées</small></div><div className="top-actions">{view!=='Accueil'&&view!=='Établissements'&&<ContextualHelp view={view}/>}<button onClick={()=>void load()} className="btn-soft" aria-label="Actualiser"><RefreshCw size={16}/></button><button onClick={()=>setModal(true)} className="btn-primary-v"><Upload size={16}/> Importer</button></div></header>
 {dash?(view==='Accueil'?(current?<HomeView dash={dash} current={current} openSignal={openSignal} setOpenSignal={setOpenSignal} onPcifSync={syncPcif} pcifSyncing={pcifSyncing}/>:<AgencyView dash={dash} onSelect={id=>setSelectedId(id)}/>):view==='Administration'?<AdminView dash={dash} reload={load}/>:view==='Établissements'?<EstablishmentsView all={all} q={q} setQ={setQ} reload={load} canManage={user.globalRole==='ADMIN'} select={id=>{setSelectedId(id);setView('Accueil')}}/>:<DomainView title={view} current={current} all={all} onSelect={id=>setSelectedId(id)}/>):<div className="loading">Construction du cockpit…</div>}</main>
