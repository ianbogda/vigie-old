...
app.post('/api/import/opale',async(req:any,reply:any)=>{try{
 const requestedEts=String((req.query as any)?.ets||'').trim();const requestedExercise=Number((req.query as any)?.exercise||0);if(!requestedEts)return reply.code(400).send({error:'Choisissez un établissement cible avant l’import.'});const targetEstablishment=await requireEstablishment(req,reply,requestedEts);if(!targetEstablishment)return;
...
app.get('/api/aged/:ets/:kind',async(req:any,reply:any)=>{try{
 ...
 const entity=String(establishment.opale_entity||ets),sourceType=kind==='clients'?'YBALAC':'YBALAF',requestedExercise=Number(req.query?.exercise||0)||null;const availableExercises=(await pool.query(`select distinct coalesce(exercise,extract(year from snapshot_date)::int) exercise from financial_snapshots where upper(opale_entity)=upper($1) and source_type=$2 order by 1 desc`,[entity,sourceType])).rows.map((r:any)=>Number(r.exercise)).filter(Number.isInteger);const effectiveExercise=requestedExercise??availableExercises[0]??null;const snap=(await pool.query(`select * from financial_snapshots where upper(opale_entity)=upper($1) and source_type=$2 and ($3::int is null or coalesce(exercise,extract(year from snapshot_date)::int)=$3) order by snapshot_date desc,created_at desc limit 1`,[entity,sourceType,effectiveExercise])).rows[0];if(!snap)return {establishment:{name:establishment.name,opaleEntity:entity},sourceType,availableExercises,effectiveExercise,snapshot:null,summary:null,rows:[]};
...
app.put('/api/admin/users/:id',async(req:any,reply:any)=>{if(!isAdmin(req))return reply.code(403).send({error:'Droit administrateur requis.'});const b=req.body||{};const q=await pool.query(`update users set first_name=$2,last_name=$3,global_role=$4,is_active=$5,updated_at=now() where id=$1 returning id,email,first_name,last_name,global_role,is_active`,[req.params.id,String(b.firstName||''),String(b.lastName||''),b.globalRole==='ADMIN'?'ADMIN':'USER',b.isActive!==false]);if(!q.rowCount)return reply.code(404).send({error:'Utilisateur introuvable.'});await audit(req,'USER_UPDATE',null,{userId:req.params.id});return {ok:true,user:q.rows[0]}});
app.post('/api/admin/users/:id/reset-password',async(req:any,reply:any)=>{if(!isAdmin(req))return reply.code(403).send({error:'Droit administrateur requis.'});const password=String(req.body?.password||'');if(password.length<12)return reply.code(400).send({error:'Le nouveau mot de passe doit contenir au moins 12 caractères.'});const q=await pool.query('update users set password_hash=$2,updated_at=now() where id=$1 returning id',[req.params.id,passwordHash(password)]);if(!q.rowCount)return reply.code(404).send({error:'Utilisateur introuvable.'});await pool.query('delete from auth_sessions where user_id=$1',[req.params.id]);await audit(req,'USER_PASSWORD_RESET',null,{userId:req.params.id});return {ok:true}});
...
app.get('/api/admin/agencies',async(req:any,reply:any)=>{if(!isAdmin(req))return reply.code(403).send({error:'Droit administrateur requis.'});return {agencies:(await pool.query(`select a.id,a.name,a.is_active,a.created_at,count(e.id)::int establishment_count from accounting_agencies a left join establishments e on e.accounting_agency_id=a.id and e.is_active group by a.id order by a.name`)).rows}});
app.post('/api/admin/agencies',async(req:any,reply:any)=>{if(!isAdmin(req))return reply.code(403).send({error:'Droit administrateur requis.'});const name=String(req.body?.name||'').trim();if(!name)return reply.code(400).send({error:'Le nom de l’agence est obligatoire.'});try{const q=await pool.query('insert into accounting_agencies(name) values($1) returning *',[name]);await audit(req,'AGENCY_CREATE',null,{agencyId:q.rows[0].id,name});return {ok:true,agency:q.rows[0]}}catch(e:any){return reply.code(409).send({error:e.code==='23505'?'Cette agence existe déjà.':e.message})}});
app.put('/api/admin/agencies/:id',async(req:any,reply:any)=>{if(!isAdmin(req))return reply.code(403).send({error:'Droit administrateur requis.'});const name=String(req.body?.name||'').trim(),active=req.body?.isActive!==false;if(!name)return reply.code(400).send({error:'Le nom de l’agence est obligatoire.'});const q=await pool.query('update accounting_agencies set name=$2,is_active=$3 where id=$1 returning *',[req.params.id,name,active]);if(!q.rowCount)return reply.code(404).send({error:'Agence introuvable.'});await audit(req,'AGENCY_UPDATE',null,{agencyId:req.params.id});return {ok:true,agency:q.rows[0]}});
app.put('/api/admin/establishments/:id/agency',async(req:any,reply:any)=>{if(!isAdmin(req))return reply.code(403).send({error:'Droit administrateur requis.'});const agencyId=req.body?.agencyId==null||req.body?.agencyId===''?null:Number(req.body.agencyId);if(agencyId!==null&&!agencyId)return reply.code(400).send({error:'Agence invalide.'});const q=await pool.query('update establishments set accounting_agency_id=$2,updated_at=now() where id=$1 returning id,uai,name,accounting_agency_id',[req.params.id,agencyId]);if(!q.rowCount)return reply.code(404).send({error:'Établissement introuvable.'});await audit(req,'ESTABLISHMENT_AGENCY_UPDATE',Number(req.params.id),{agencyId});return {ok:true,establishment:q.rows[0]}});
